...
CREATE POLICY "Users can view complaints in their branch"
ON public.complaints FOR SELECT
TO authenticated
USING (branch = public.get_user_branch(auth.uid()));

CREATE POLICY "Users can create complaints in their branch"
ON public.complaints FOR INSERT
TO authenticated
WITH CHECK (
  user_id = auth.uid()
  AND branch = public.get_user_branch(auth.uid())
);
...
-- Votes RLS Policies
CREATE POLICY "Users can view all votes in their branch complaints"
...
USING (
  EXISTS (
    ...
    AND c.branch = public.get_user_branch(auth.uid())
  )
);
